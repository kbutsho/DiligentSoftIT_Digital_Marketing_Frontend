...
const DashboardTeamPage = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const response = await axios.get(`${CONFIG.BackendURL}/api/team`, { params: data });
    ...
    const handleFormSubmit = async (event) => {
        ...
        try {
            ...
            const response = await axios.post(`${CONFIG.BackendURL}/api/team/create`, postData,
                {
                    headers: {
    ...
    const handleDelete = async () => {
        try {
            const token = Cookies.get('token');
            const response = await axios.delete(`${CONFIG.BackendURL}/api/team/${deleteItem?.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleFormUpdate = async (e) => {
        ...
        postData.append('designation', updateItem.designation);
        postData.append('facebook', updateItem.facebook);
        postData.append('twitter', updateItem.twitter);
        postData.append('linkedin', updateItem.linkedin);
        postData.append('instagram', updateItem.instagram);
        if (updateItem.image) {
        ...
        try {
            ...
            const response = await axios.post(`${CONFIG.BackendURL}/api/team/update`, postData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        'Content-Type': 'multipart/form-data',
    ...
    return (
        <div className="py-4">
            <h4 className="mb-4">Team Member List</h4>
            ...
            <div className="d-flex flex-wrap align-items-center gap-3 mb-4">
                ...
                <div>
                    <button className="btn btn-outline-primary btn-sm py-2" onClick={toggleAddModal}>Add Team Member</button>
                </div>
            </div>
                        ...
                        <table className="table table-bordered table-sm table-hover" style={{ fontSize: "14px" }}>
                            <thead className="table-light">
                                <tr>
                                    ...
                                    <th className="text-center pb-2">Facebook</th>
                                    <th className="text-center pb-2">Instagram</th>
                                    <th className="text-center pb-2">Twitter</th>
                                    <th className="text-center pb-2">LinkedIn</th>
                            ...
                            <tbody>
                                {data.map((item, index) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className='text-center'>{item.facebook ?? <i className="fas fa-minus"></i>}</td>
                                        <td className='text-center'>{item.instagram ?? <i className="fas fa-minus"></i>}</td>
                                        <td className='text-center'>{item.twitter ?? <i className="fas fa-minus"></i>}</td>
                                        <td className='text-center'>{item.linkedin ?? <i className="fas fa-minus"></i>}</td>
                    ...
                    <Modal.Body>
                        <div className="p-2">
                            <div className="d-flex justify-content-between py-3">
                                <h4 className="text-uppercase fw-bold">Add Team Member</h4>
                                ...
                                <div className="row g-3">
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        <input
                                            ...
                                            onChange={handleChange}
                                            className={`form-control ${formData?.errors?.designation ? 'is-invalid' : ''}`}
                                        />
                                        <div className="invalid-feedback">{formData?.errors?.designation}</div>
                                    </div>
                                ...
                                <div className="row g-3">
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        <input
                                            name="designation"
                                            type="text"
                                            value={updateItem.designation}
                                            placeholder="e.g., Developer"
                                            onChange={handleUpdateFormChange}
                                            className={`form-control ${updateItem?.errors?.designation ? 'is-invalid' : ''}`}
                                        />
                                        <div className="invalid-feedback">{updateItem?.errors?.designation}</div>
                                    </div>
